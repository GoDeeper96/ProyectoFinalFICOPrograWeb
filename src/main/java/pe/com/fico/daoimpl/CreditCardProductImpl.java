...
public class CreditCardProductImpl implements ICreditCardProductDao {
	...
	@Transactional
	public void erase(int idCredit) {
		CreditCardProduct ccp=new CreditCardProduct();
		try {
			ccp=em.getReference(CreditCardProduct.class, idCredit);
			em.remove(ccp);
		}catch(Exception e) {
	...
	@Transactional
	public void update(CreditCardProduct cpp) {
		try {
			em.merge(cpp);
		}catch(Exception e) {
			System.out.println(e.getMessage());
